...
| `kairos_profile` | **Default `default-kairos`.** Namespaces the build artifact and BCM-side state (`<profile>-installer` software image, `<profile>` category, `/cm/shared/kairos/<profile>/` upload). Multiple profiles coexist on the same BCM — pick a different name to keep multiple builds side-by-side | `"gpu-kairos"` |
