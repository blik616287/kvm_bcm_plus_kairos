#!/usr/bin/env bash
# Interim render-then-lint bridge for bash that still lives inside Jinja2
# *.sh.j2 templates (the linter can't parse {{ }} / {% %}).
# (Comment lines here avoid starting with the word that names the bash linter,
#  since a leading "# <that-word>" is parsed as a malformed directive — SC1073.)
#
...
# lint findings respect the severity and DO set the exit code.
set -uo pipefail
...
command -v ansible > /dev/null 2>&1 || {
    echo "ansible not found — cannot render templates"
    exit 2
}
